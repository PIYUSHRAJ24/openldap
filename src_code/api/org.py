...
@bp.route('/list_user_dept_sec', methods=['GET'])
def list_user_dept_sec():
    try:
        data =[]
        for a in g.org_access_rules:
            if a.get('rule_id') != None and  a.get('is_active') == "Y" and a['digilockerid'] == g.digilockerid:
                data.append({
                    ...
                    'departments': g.dept_details.get(a.get('dept_id'),{}).get('name',''),
                    'sections': g.dept_details.get(a.get('sec_id'),{}).get('name',''),
                 })
        return {STATUS: SUCCESS, RESPONSE: data}, 200
    except Exception as e:
        res = {STATUS: ERROR, ERROR_DES: "get_role: " + str(e)}
        logarray.update({RESPONSE: res})
        RABBITMQ_LOGSTASH.send_to_queue(logarray, 'Logstash_Xchange', 'org_logs_')
        return res, 400
...
def get_details():
    ...
    try:
        ...
        res['current_user_response'] = {
            'digilockerid': g.digilockerid,
            **Roles.rule_id(g.role),
            **CommonLib.get_profile_details({'digilockerid': g.digilockerid}),
            'departments': g.dept_details,
            'sections': g.sec_details
            # list_sub_entities
        }
...
def grant_access():
    ...
    try:
        ...
        if rule_id == 'ORGR001' and designation == "director":
            ...
            res, status_code = verify_mobile_otpRABBITMQ.send_to_queue({"data": data1}, 'Organization_Xchange', 'org_update_details_')
            if status_code != 200:
                logarray.update({RESPONSE: {STATUS: ERROR, RESPONSE: res.pop(RESPONSE) if res.get(RESPONSE) else res}})
                RABBITMQ_LOGSTASH.log_stash_logeer(logarray, logs_queue, g.endpoint)
                return res, status_code
