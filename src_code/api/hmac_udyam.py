...
bp = Blueprint("hmac_udyam", __name__)
logarray = {}
CONFIG = dict(CONFIG)
...
def validate_user():
    ...
    try:
        ...
        return {STATUS: ERROR, ERROR_DES: Errors.error('err_1201')+"[#12300]"}, 401
...
def update_udyam():
    ...
    try:
        ...
        RABBITMQ_LOGSTASH.log_stash_logeer(logarray, logs_queue, 'update_udyam')
        VALIDATIONS.log_exception(e)
        return jsonify({"status": "error", "error_description": Errors.error('err_1201')+"[#12301]"}), 400
...
def ids_udyam_verify(udyam_no, mobile):
    try:
        ...
        RABBITMQ.send_to_queue(logarray, 'Logstash_Xchange', 'entity_auth_logs_')
        VALIDATIONS.log_exception(e)
        return {"status": "error", 'error_description': Errors.error('err_1201')+"[#12302]"}, 500
