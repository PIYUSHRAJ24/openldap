...
class MongoLib:
    ...
    def devices_eve(self, locker_id, collection, query, sort={}, limit=10):
        try:
            ...
            eve_query= "/"+collection+"?where="+json.dumps(query)+"&max_results="+str(limit)+"&sort="+json.dumps(sort)
            eve_url = accounts_eve["url"]+eve_query
            print(eve_url)
            get_eve_data = get(eve_url, auth=HTTPBasicAuth(accounts_eve["username"], accounts_eve["password"]))                                    
    ...
    def devices_entity_lockerid_eve(self, locker_id, entity_id, collection, query, sort={}, limit=10):
        try:
    ...
    def devices_entity_eve_by_entity_id(self, entity_id, collection, query, sort={}, limit=10):
        try:
    ...
    def accounts_eve(self, collection, query, projection, sort={}, limit=50):
        try:
            ...
            eve_query = '/' + collection + '?where=' + json.dumps(query) + '&projection=' + json.dumps(projection) + \
                '&max_results=' + str(limit) + '&sort=' + json.dumps(sort)
            eve_url = accounts_eve['url'] + eve_query
            get_eve_data = get(eve_url, auth=HTTPBasicAuth(
                accounts_eve['username'], accounts_eve['password']))
            eve_resp_data = json.loads(get_eve_data.content)
            if eve_resp_data.get("_meta") and eve_resp_data["_meta"]['total'] > 0:
                def pop(data):
                    data.pop("_id")
    ...
    def org_eve(self, collection, query, projection, sort={}, limit=10):
        try:
            ...
            get_eve_data = get(eve_url, auth=HTTPBasicAuth(org_eve['username'], org_eve['password']))
            eve_resp_data = json.loads(get_eve_data.content)
            if eve_resp_data.get("_meta") and eve_resp_data["_meta"]['total'] > 0:
                def pop(data):
                    try:
    ...
    def org_eve_post(self, collection, data):
        try:
            ...
            response = post(eve_url, data=json.dumps(data), headers={"Content-Type":"application/json"} , auth=HTTPBasicAuth(CONFIG['org_eve']['username'], CONFIG['org_eve']['password']))
            res = json.loads(response.text)
            if response.status_code >= 200 and response.status_code < 300:
                return {STATUS: SUCCESS, MESSAGE: Messages.message("MSG_109")}, 200
            else:
                return {STATUS: ERROR, ERROR_DES: Errors.error("ERR_MSG_111"), RESPONSE: res["_error"]["message"]}, response.status_code
        except Exception as e:
            return {
                STATUS: ERROR,
                ERROR_DES: Errors.error("ERR_MSG_111"),
    ...
    def org_eve_patch(self, collection, data):
        try:
            eve_url = org_eve['url'] + collection
            response = patch(eve_url, data=json.dumps(data), headers={"Content-Type":"application/json"} , auth=HTTPBasicAuth(CONFIG['org_eve']['username'], CONFIG['org_eve']['password']))
            res = json.loads(response.text)
            if response.status_code >= 200 and response.status_code < 300:
                return {STATUS: SUCCESS, MESSAGE: Messages.message("MSG_109")}, 200
            else:
    ...
    def set_pin(self, digilockerid, pin):
        try:
            ...
            if res is not None:
                ...
                return {"status": "error", "error_description": "Login pin can not be updated!"}
    ...
    def match_pin(self, pin, hash):
        try:
            if pin != '' and hash:
                ...
                return {"status": "error", "error_description":"Some Technical Error occured."}
        except Exception as e:
            return {"status": "error", "error_description": str(e)}
    ...
    def pin_validation(self, pin):
        try:
            ...
            if valid_otp:
                ...
                return 400, {'status': 'error', 'error_description': 'Please enter valid PIN.'}
        except Exception as e:
            return 400, {'status': 'error', 'error_description': 'Invalid pin parameter.'}
